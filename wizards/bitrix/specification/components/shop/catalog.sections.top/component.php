...
$arParams["SHOW_NEW"] = $arParams["SHOW_NEW"]=="Y";
...
if($this->StartResultCache(false, array($arrFilter, CDBResult::NavStringForCache($arParams["PAGE_ELEMENT_COUNT"]), ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	while($arSection = $rsSections->GetNext())
	{
		...
		$arrFilter_new["ACTIVE"] = "Y";
		$arrFilter_new["IBLOCK_ID"] = $arParams["IBLOCK_ID"];
		$arrFilter_new["SECTION_ID"] = $arParams["SECTION_ID"];
		$arrFilter_new["INCLUDE_SUBSECTIONS"] = "Y";
		$arrFilter_new["ACTIVE_DATE"] = "Y";
		$arrFilter_new["CHECK_PERMISSIONS"] = "Y";
		//PRICES
		if(!$arParams["USE_PRICE_COUNT"])
		{
			foreach($arResult["PRICES"] as $key => $value)
		...
		$arSort = array(
			$arParams["created"] => "DESC",
			"ID" => "DESC",
		);
		
		$rsElements_all = CIBlockElement::GetList($arSort, $arrFilter_new, false, array("nTopCount"=>$arParams["ELEMENT_COUNT"]), $arSelect);
		$rsElements_all->SetUrlTemplates($arParams["DETAIL_URL"]);
		$arAllProducts["ITEMS"] = array();
		while($obElement = $rsElements_all->GetNextElement())
		{
				$arItem = $obElement->GetFields();
				$arButtons = CIBlock::GetPanelButtons(
				$arItem["IBLOCK_ID"],
			...
			$arNewProducts["ITEMS"][]=$arItem;
		}
		$arResult["NEW"]=$arNewProducts;
		}
		...
		$rsElements_all = CIBlockElement::GetList($arSort, $arrFilter, false, array("nTopCount"=>$arParams["ELEMENT_COUNT"]), $arSelect);
		$rsElements_all->SetUrlTemplates($arParams["DETAIL_URL"]);
		$arAllProducts["ITEMS"] = array();
		while($obElement = $rsElements_all->GetNextElement())
		{
			...
			$arAllProducts["ITEMS"][]=$arItem;
		}
		$arResult["ALL"]=$arAllProducts;
		}
		/*------------------------------------------------------------------------------------------------*/
...
{
	$APPLICATION->AddChainItem($arPath['NAME'], $arPath['LIST_PAGE_URL']);
	$arResult["SECTION"]["PATH"][]=$arPath;
}
$path = $arResult["SECTION"]["PATH"];
if($arParams["SET_TITLE"])
	$APPLICATION->SetTitle($arResult["SECTION"]["PATH"][count($arResult["SECTION"]["PATH"])-1]["NAME"]);
?>
