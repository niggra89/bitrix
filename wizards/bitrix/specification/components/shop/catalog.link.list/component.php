...
if (!function_exists('getProp')) { 
	...
	{
		$resultProp = Array();
		foreach($prop as $prop_elem)
		{
			$res = CIBlockElement::GetByID($prop_elem);
			if($ar_res = $res->GetNext())
			{
				$sect = CIBlockSection::GetByID($ar_res["IBLOCK_SECTION_ID"]);
				if($ar_sect_res = $sect->GetNext())
				{			
					
					$top_level = CIBlockSection::GetByID($ar_sect_res["IBLOCK_SECTION_ID"]);
					if($ar_top_level = $top_level->GetNext())
					{
						$resultProp[$ar_top_level["ID"]]["NAME"] = $ar_top_level["NAME"];
						$resultProp[$ar_top_level["ID"]]["VALUES"][] =  Array(
							"NAME" => $ar_sect_res["NAME"],
							"NAME_ID" => $ar_sect_res["ID"],
							"VALUE" => $ar_res["NAME"],
							"VALUE_ID" => $ar_res["ID"],
							);
					}
...
if (!function_exists('check_link_')) { 
	function check_link_($id)
	{
		...
		$rs=CIBlockElement::GetList(array(), array("ACTIVE"=>"Y", "IBLOCK_ID"=>$iblock_configure_id,"PROPERTY_products"=>$id), false, array("nTopCount"=>1), array("ID","PROPERTY_products","PROPERTY_specification","PROPERTY_required","PROPERTY_filter","PROPERTY_analog")); 
		if($ar=$rs->GetNext()) 
		{ 
			return $ar; 
		}
...
if (!function_exists('getProps')) { 
	function getProps($id,$block_id,$iblock_specfication_id){
		$elem_values = array(); 
		$elem_res = CIBlockElement::GetProperty($block_id,$id,Array(),Array("CODE"=>"specification"));
		while($ar_elem_res = $elem_res->GetNext())
		{
			$elem_values[] = $ar_elem_res['VALUE']; 
		}		
		$sect = 0;
		$res = CIBlockElement::GetByID($id);
		if($ar_res = $res->GetNext())
		{
			$sect = $ar_res["IBLOCK_SECTION_ID"];
		}
		else return false;
		while(!($link_res = check_link_($sect))&&$sect)
		{
		...
		{			
			foreach($link_res["PROPERTY_ANALOG_VALUE"] as $analog){
				$res4=CIBlockElement::GetList(array(), array("ACTIVE"=>"Y", "IBLOCK_ID"=>$iblock_specfication_id,"SECTION_ID"=>$analog), false); 		
				$val = Array();
				while($ar_res4 = $res4->GetNext())
				{
					$val[]=$ar_res4["ID"];				
				}
				$result = array_merge($result,array_intersect($elem_values ,$val));
			}
		}
		return $result;
	}
...
$resIB = CIBlock::GetByID($arParams["IBLOCK_ID"]);
if($ar_res = $resIB->GetNext())
{
	$version = $ar_res["VERSION"];
}
...
$arrFilterSpecification_ = getProps($arParams["ELEMENT_ID"], $arParams["IBLOCK_ID"], $arParams["IBLOCK_SPECIFICATION_ID"]);

$arrFilterSpecification=Array("LOGIC"=>"OR");
foreach($arrFilterSpecification_ as $fv)
{
	$arrFilterSpecification[]= array("PROPERTY_specification" => $fv);
}

if($arParams["SECTION_ID"]==null){
...
/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, array($arrFilter, $arrFilterSpecification_, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	if(!CModule::IncludeModule("iblock"))
	{
	...
	//EXECUTE
	$rsElements = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter,Array($arrFilterSpecification)), false, false, array_merge($arSelect,Array("PROPERTY_specification")));
	$rsElements->SetUrlTemplates($arParams["DETAIL_URL"]);
	if($arParams["BY_LINK"]!=="Y" && !$arParams["SHOW_ALL_WO_SECTION"])
		$rsElements->SetSectionContext($arResult);
	$arResult["ITEMS"] = array();
	$count=0;
	while(($obElement = $rsElements->GetNextElement()) && ($count<$arParams["PAGE_ELEMENT_COUNT"]))
	{
		$arItem = $obElement->GetFields();

		$specification = Array();
		if($version == 2)$specification = $arItem["PROPERTY_SPECIFICATION_VALUE"];
		else
		{
			$db_props = CIBlockElement::GetProperty($arItem["IBLOCK_ID"], $arItem["ID"], array("sort" => "asc"), Array("CODE"=>"specification"));
			while($ar_props = $db_props->Fetch()){
				$specification[] = $ar_props["VALUE"];
			}
		}
		if(arr_in_arr($arrFilterSpecification_,$specification) && !in_array($arItem["ID"],$arResult["ELEMENTS"]))
		{
		if($arResult["ID"])
	...
	$rs->NavStart($arParams["PAGE_ELEMENT_COUNT"]);
