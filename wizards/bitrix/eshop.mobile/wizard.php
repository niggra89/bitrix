...
class SelectSiteStep extends CSelectSiteWizardStep
{
	function InitStep()
	{
		...
		$wizard->solutionName = "esop.mobile";
...
class SiteSettingsStep extends CSiteSettingsWizardStep
{
	...
	{
		$wizard =& $this->GetWizard();
		$wizard->solutionName = "ehop.mobile";
		parent::InitStep();
		...
		$siteName = COption::GetOptionString("eshop", "siteName", GetMessage("WIZ_COMPANY_NAME_DEF"), $siteID);
		if(strlen($siteName) <= 0)
			$siteName = GetMessage("WIZ_COMPANY_NAME_DEF");
		$wizard->SetDefaultVars(
			Array(
				"siteName" => $siteName,
				"siteTelephone" => COption::GetOptionString("eshop", "siteTelephone", GetMessage("WIZ_COMPANY_TELEPHONE_DEF"), $siteID),
				"shopEmail" => COption::GetOptionString("eshop", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"siteLogo" => "/bitrix/wizards/bitrix/eshop.mobile/site/public/".LANGUAGE_ID."/images/logo.jpg",
			)
		);
	}
