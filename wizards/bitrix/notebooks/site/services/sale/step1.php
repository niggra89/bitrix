...
if (CModule::IncludeModule("catalog")){
	if(!CCatalogGroup::GetBaseGroup())
	{
		...
		if (isset($arCurrentPriceTypes["BASE"])){
			CCatalogGroup::Update($arCurrentPriceTypes["BASE"]["ID"],array_merge($arCurrentPriceTypes["BASE"],array("BASE"=>"Y")));
		}
		else
		{
			$arFields = array(
	...
	else
	{
		...
		$db_res = CCatalogGroup::GetGroupsList(array("GROUP_ID"=>2, "BUY"=>"Y", "CATALOG_GROUP_ID" => $baseId));
		if (!$ar_res = $db_res->Fetch())
		{
			$rn = CCatalogGroup::GetByID($baseId);
			$arFields = array(
			   "NAME" => $rn["NAME"],
			   "SORT" => $rn["SORT"],
			   "USER_GROUP" => array(2),  
			...
			if (!CCatalogGroup::Update($baseId, $arFields))
			   echo "Error";			
		}
	}
}	
	
$dbPerson = CSalePersonType::GetList(Array(),Array("LID" => $siteID));
