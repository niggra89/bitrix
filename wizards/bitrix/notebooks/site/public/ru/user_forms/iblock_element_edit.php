...
<script>
var params = {"required":[],"filter":[],"analog":[]};
...
function select_field(selectName)
{
	
	var parent_id = $('#specification option:selected').attr('value');
	
	$('#'+selectName+" option").each(function(){$(this).remove();});
	
	var options = "";
	for(var i=0;i<params[selectName].length;i++){
		var opt = params[selectName][i];
	
		if(opt.top_level==parent_id){
		
			options+="<optgroup value='"+opt.value+"' '"+opt.selected+"' label='"+opt.display_value+"' />";
			
			for(var j=0;j<params[selectName].length;j++){
				var child_opt = params[selectName][j];
				if(child_opt.top_level==opt.value)
				{
					options+="<option value='"+child_opt.value+"' '"+child_opt.selected+"' >"+child_opt.display_value+"</option>";
				}
			}
			
		}
	}
	$('select#'+selectName).html(options);
}
$(document).ready(function(){	
	$('#specification').bind('change',function(){
...
<?
function _ShowPropertyField_($name,$property_fields,$id)
{
	//echo "<pre>";print_r($property_fields);echo "</pre><br/><br/>";
...
$values = $property_fields["VALUE"];
	...
	$res = "";
	$bWas = false;
	$filter_top_level = Array("IBLOCK_ID"=>$property_fields["LINK_IBLOCK_ID"]);
	if($property_fields["CODE"]=="products"){$filter_top_level["DEPTH_LEVEL"]="";}
	elseif($property_fields["CODE"]=="specification"){$filter_top_level["DEPTH_LEVEL"]=1;}
	else
	{

		$filter_top_level[">DEPTH_LEVEL"]=1;
	...
	{
	//echo "<pre>";print_r($ar["IBLOCK_SECTION_ID"]);echo "</pre><br/><br/>";
		//if($ar["IBLOCK_SECTION_ID"]!=$id){}
		
		$del=false;
		$db_props = CIBlockElement::GetProperty($property_fields["IBLOCK_ID"], $id, "sort", "asc", Array("CODE"=>"specification"));
		if ($ob = $db_props->GetNext()) 
		{    
			if($ar["IBLOCK_SECTION_ID"]!=$ob['VALUE']&&$property_fields["CODE"]!="specification"&&$property_fields["CODE"]!="products")
			{	
				$del = true;		
			}
		}
		if(!$del&&($property_fields["CODE"]!="products"&&$property_fields["CODE"]!="specification"))
		{
			
			$parent_options[]=$ar["ID"];
			$res .= '<optgroup value="'.$ar["ID"].'" class="top_level_'.$ar["IBLOCK_SECTION_ID"].'"  label="'.$ar["NAME"].'" style="font-style:normal;"></optgroup>';
		}
		
		if(in_array($ar["IBLOCK_SECTION_ID"], $parent_options)||($property_fields["CODE"]=="products")||($property_fields["CODE"]=="specification"))
		{
			...
			$res .= ' class="top_level_'.$ar["IBLOCK_SECTION_ID"].'"  >'.str_repeat(" . ", $ar["DEPTH_LEVEL"]-1).$ar["NAME"].'</option>';
			
		}
		
		if($property_fields["CODE"]!="specification"&&$property_fields["CODE"]!="products")
		{
			?>
			<script>
				var option = {"value":<?=$ar["ID"]?>,"selected":"<?if(in_array($ar["ID"], $values)){echo " selected ";}else{ echo "";}?>","top_level":"<?=$ar["IBLOCK_SECTION_ID"]?>","display_value":"<?=str_repeat(" . ", $ar["DEPTH_LEVEL"]-1).$ar["NAME"]?>"};
				addParam("<?=$property_fields["CODE"]?>",option);
				//alert("<?=$property_fields["CODE"]?>");
			</script>
			<?
		}
	}
	
	echo '<select id="'.$property_fields["CODE"].'" name="'.$name.'[]" size="'.$property_fields["MULTIPLE_CNT"].'" '.($property_fields["MULTIPLE"]=="Y"?"multiple":"").'>';
...
if(count($PROP)>0):
	...
	$tabControl->AddSection("IBLOCK_ELEMENT_PROP_VALUE", "Настройка товарного каталога");
	
	$i=0;
	foreach($PROP as $prop_code=>$prop_fields):
		if($i==2)
		{
			$tabControl->AddSection("IBLOCK_ELEMENT_PROP_VALUE2", "Дополнительно");
		}
		...
		<tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
			...
			<td><?_ShowPropertyField_('PROP['.$prop_fields["ID"].']',$prop_fields,$ID);?></td>
		</tr>
		<?
			$hidden = "";
