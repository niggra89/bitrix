...
$(document).ready(function(){
	$('a.PROPERTY_SPECIFICATION_BLOCK').bind('click',function(){
		var blockId = $(this).attr('id');
		if($('div#'+blockId).css('display')=='none')
		{
			$('div#'+blockId).css('display','block');
		}
		else
		{
			$('div#'+blockId).css('display','none');
		}
		return false;
	});
});
...
function check_link($id)
{
    ...
    while($ar_res = $res->Fetch())
    {
       ...
       $rs=CIBlockElement::GetList(array(), array("ACTIVE"=>"Y", "IBLOCK_ID"=>$iblock_configure_id,"PROPERTY_products"=>$id), false, array("nTopCount"=>1), array("ID","PROPERTY_specification","PROPERTY_required")); 
	if($ar=$rs->GetNext()) 
	{ 
		return $ar; 
	}
...
{
	$catalog_section_id = $id;

	$sect=$catalog_section_id;
	//echo $sect;
	...
	$key = 0;
	if(!$link_res){
		echo "Набор характеристик не определен.";
		return;
	}

	//echo "<pre>";print_r($link_res);echo "</pre>";
	...
	$res2 = CIBlockSection::GetList(Array(), Array("SECTION_ID"=>$specificId), false);
	while($ar_res2 = $res2->GetNext())
	{
		echo "<a href='#' style='font-size:13px;text-decoration:none;border-bottom:1px dotted  #2747BF;' class='PROPERTY_SPECIFICATION_BLOCK' id='PROPERTY_SPECIFICATION_BLOCK".$ar_res2["ID"]."'>".$ar_res2["NAME"]."</a><br/><br/><div id='PROPERTY_SPECIFICATION_BLOCK".$ar_res2["ID"]."' style='width:400px;'>";
		$res3 = CIBlockSection::GetList(Array(), Array("SECTION_ID"=>$ar_res2["ID"]), false);
		while($ar_res3 = $res3->GetNext())
		{
			$req = "";
			if(in_array($ar_res3["ID"],$arr_req)){
				$req = "<font style='color:red;'>*</font>";
			}
			echo "<div style='float:left;width:170px;'> ".$req.$ar_res3["NAME"]."</div>";
			echo "<select name='".$name."[".$ar_res3["ID"]."]' style='width:200px;'>";
...
if($res_arr = $prop_c_res->Fetch()){
   $spec_prop_id = $res_arr["ID"];
			...
			$res4=CIBlockElement::GetList(array(), array("ACTIVE"=>"Y", "IBLOCK_ID"=>$iblock_spec_id,"SECTION_ID"=>$ar_res3["ID"]), false); 
			while($ar_res4 = $res4->GetNext())
			{
				$selected="";
				if(in_array($ar_res4["ID"],$property_fields["VALUE"])||(in_array($ar_res4["ID"],$_POST["PROP"][$spec_prop_id]))){$selected=" selected ";}
				echo "<option ".$selected." value=".$ar_res4["ID"].">".$ar_res4["NAME"]."</option>";
			}
			echo "</select>";
			
			//echo '<input name="'.$name.'['.$key.']" id="'.$name.'['.$key.']" value="'.htmlspecialcharsex($val).'" type="hidden">';
			echo "<br/><br/>";
			$key++;
		}
		echo "</div>";
...
if(count($PROP)>0):
	...
	foreach($PROP as $prop_code=>$prop_fields):
		...
		<tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
			...
			<?if($prop_fields["CODE"]=="specification"){
				?>
				<td valign="top" colspan=2 align="center">
				<?
				_ShowPropertyField_('PROP['.$prop_fields["ID"].']',$prop_fields,$str_IBLOCK_ELEMENT_SECTION[0]);
			}
			else
			{?>
