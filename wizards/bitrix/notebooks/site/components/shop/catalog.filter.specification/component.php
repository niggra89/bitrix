...
function getSpecification($section,$selected,$iblock_id,$iblock_specfication_id, $prices_interval,$IBLOCK_PRICE_CODE){
	...
	else
	{
		...
		{ 
			if(arr_in_arr_f($selected,$arElement["PROPERTY_SPECIFICATION_VALUE"])&&(($arElement["CATALOG_PRICE_".$IBLOCK_PRICE_CODE] >= $prices_interval["FROM"]&&$arElement["CATALOG_PRICE_".$IBLOCK_PRICE_CODE] <= $prices_interval["TO"])||(!$prices_interval["FROM"])))
			{
				$res_prop_visible = array_unique(array_merge($res_prop_visible,$arElement["PROPERTY_SPECIFICATION_VALUE"]));
			}
		...
		while($ar_res2 = $res2->GetNext())
		{
			...
			while($ar_res3 = $res3->GetNext())
			{
				...
				if(count($val)>0 && in_array($ar_res3["ID"],$link_res["PROPERTY_FILTER_VALUE"]))$spec_elem[]=Array("NAME"=>$ar_res3["NAME"],"ID"=>$ar_res3["ID"],"SELECTED"=> $s,"VALUE"=>$val);			
			}
