...
if(is_array($arCompare) && count($arCompare)>0)
{
	...
	/*=======================================================================================================================*/
	$compare_specification = Array();
	foreach($arResult["ITEMS"] as $compare_elem)
	{
		$section_id;$block_id;
		$res = CIBlockElement::GetByID($compare_elem["ID"]);
		...
		if($link_res)
		{
			$res_prop = array();
			$result = Array();
			$db_props = CIBlockElement::GetProperty($block_id, $compare_elem["ID"], array("sort" => "asc"), Array("CODE"=>"specification"));
			...
			while($ar_res2 = $res2->GetNext())
			{
				...
				{
					$compare_specification[$ar_res2["ID"]]["VALUES"][$ar_res3["ID"]]["ID"] = $ar_res3["ID"];
					$compare_specification[$ar_res2["ID"]]["VALUES"][$ar_res3["ID"]]["NAME"] = $ar_res3["NAME"];

					$res4=CIBlockElement::GetList(array(), array("ACTIVE"=>"Y", "IBLOCK_ID"=>$arParams["IBLOCK_SPECIFICATION_ID"],"SECTION_ID"=>$ar_res3["ID"]), false); 
						
					while($ar_res4 = $res4->GetNext())
					{
						if(in_array($ar_res4["ID"],$res_prop))
						{
							$val[$ar_res3["ID"]] = Array("NAME"=>$ar_res3["NAME"],"VALUE"=>$ar_res4["NAME"]);
							$compare_specification[$ar_res2["ID"]]["VALUES"][$ar_res3["ID"]]["VALUES"][$compare_elem["ID"]]	= $ar_res4["NAME"];						
						}					
					}							
				}				
				if(count($val)>0)
				{
					$compare_specification[$ar_res2["ID"]]["ID"] = $ar_res2["ID"];
					$compare_specification[$ar_res2["ID"]]["NAME"] = $ar_res2["NAME"];	
				}
			}
		}
	}
	$arResult["SPECIFICATION"] = $compare_specification;
	/*=======================================================================================================================*/
