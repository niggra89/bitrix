		...
		while($prop = $props->GetNext()):
			...
			<dl>
				...
				<?if($prop['TYPE']=='LOCATION'):
                    ...
                    <dd>
                        <div class="select-box location">
                            <select class="styled" name="<?=$prop['CODE']?>">
                ...
                <?elseif($prop['TYPE']=="TEXTAREA"):?>
					<dd><textarea name="<?=$prop['CODE']?>" class="inp-text_ia style-1_ia"><?=$_COOKIE[$prop['CODE']]!=''?$_COOKIE[$prop['CODE']]:$user[$prop['CODE']]?></textarea></dd>
				<?else:?>
					<dd><input type="text" value="<?=$_COOKIE[$prop['CODE']]!=''?$_COOKIE[$prop['CODE']]:$user[$prop['CODE']]?>" name="<?=$prop['CODE']?>" class="inp-text_ia style-1_ia"></dd>
				<?endif;?>
			</dl>
		<?endwhile;?>
	<?endwhile;?>
