...
while($prop = $list->GetNext()) if(isset($_POST[$prop['CODE']])){
	if($prop['TYPE']=='LOCATION'){		
		$location = $_POST[$prop['CODE']];
	}elseif($prop['IS_NAME']=='Y'){
		$username = $_POST[$prop['CODE']];
	}elseif($prop['IS_EMAIL']=='Y'){
		$email = $_POST[$prop['CODE']];
		if($prop['REQUIED']=='Y') $emailreq = true;
	}elseif($prop['REQUIED']=='Y' && $_POST[$prop['CODE']]==''){
		$errors[] = GetMessage('FILL_FIELD').' &laquo;'.$prop['NAME'].'&raquo;';
	}
	if(preg_match("#phone#i",$prop['CODE'])){
		$phone = $_POST[$prop['CODE']];
		if($prop['REQUIED']=='Y') $phonereq = true;
	}
	if($prop['CODE']!='EMAIL') $userArr[$prop['CODE']] = $_POST[$prop['CODE']];
}
...
elseif($phone!='' && strlen($phone_chk)<5) $errors[] = GetMessage('PHONE_TOO_SHORT');
...
if(sizeof($errors) > 0){
	...
	if($USER->IsAuthorized()){
		...
		if(!$usr){
			...
			if($_SERVER['SERVER_NAME']=="www.nachisto.ru" && !in_array(1, $USER->GetUserGroup($usr['ID']))){
	...
	if($price >= 0){
		...
		$orderVals = array(
				...
				'PRICE_DELIVERY'=>$dt['PRICE'] ,// - стоимость доставки заказа; 
				...
				'PRICE'=>$price+$dt['PRICE']+$proc ,// - общая стоимость заказа; 
				'CURRENCY'=>'RUB' ,// - валюта стоимости заказа; 
				'DISCOUNT_VALUE'=>$disc ,// - общая величина скидки; 
			...
			while($prop = $list->GetNext()){
				if($_POST[$prop['CODE']]!=""){					
					if($prop['IS_LOCATION']=='Y'){
						...
						CSaleOrderPropsValue::Add(array("ORDER_ID"=>$orderId,"ORDER_PROPS_ID"=>$prop['ID'],"NAME"=>$prop['NAME'],"CODE"=>$prop['CODE'],"VALUE"=>$_POST[$prop['CODE']]));
					}
				}
			}
...
";
			...
			*/
			// Отправляем письмо
			CEvent::SendImmediate("SALE_NEW_ORDER","s1",Array(
				"ORDER_ID"=>$orderId,
