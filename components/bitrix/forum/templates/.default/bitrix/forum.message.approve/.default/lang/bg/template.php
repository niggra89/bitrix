<?
$MESS ['F_TITLE'] = "Скрити съобщения";
$MESS ['F_AUTHOR_PROFILE'] = "Профил на автора";
$MESS ['F_NUM_MESS'] = "Съобщения:";
$MESS ['F_DATE_REGISTER'] = "Регистрация:";
$MESS ['F_EMPTY_RESULT'] = "Скрити съобщения не са намерени.";
$MESS ['F_PRIVATE_MESSAGE'] = "Съобщение";
$MESS ['F_ANCHOR'] = "Линк на това съобщение";
$MESS ['F_ANCHOR_TITLE'] = "Копирайте в буфера адреса на линка на това съобщение";
$MESS ['F_ATTACH_FILES'] = "Прикачени файлове";
$MESS ['F_POINTS'] = "Оценки:";
$MESS ['F_PRIVATE_MESSAGE_TITLE'] = "Изпрати лично съобщение до автора";
$MESS ['F_EMAIL_TITLE'] = "Изпрати писмо на автора на съобщението по E-Mail";
$MESS ['F_DELETE'] = "Премахни";
$MESS ['F_DELETE_CONFIRM'] = "Сигурни ли сте, че искате да изтриете съобщението? Тази операция не може да бъде възстановена!";
$MESS ['F_DELETE_MESSAGES'] = "Премахни съобщенията";
$MESS ['F_DELETE_MESSAGES_CONFIRM'] = "Сигурни ли сте, че искате да изтриете съобщенията? Тази операция не може да бъде възстановена!";
$MESS ['F_EDIT'] = "Редактирай";
$MESS ['F_EDIT_HEAD'] = "Редактирано:";
$MESS ['F_MANAGE_MESSAGES'] = "Управление на съобщенията";
$MESS ['F_REAL_IP'] = "/ реален";
$MESS ['F_SHOW'] = "Покажи";
$MESS ['F_SHOW_MESSAGES'] = "Покажи съобщенията";
$MESS ['JS_NO_MESSAGES'] = "Не е посочен списък на съобщенията. Изберете съобщения.";
$MESS ['JS_NO_ACTION'] = "Не е избрано действие. Изберете действие.";
$MESS ['F_USER_ID'] = "ID на посетителя";
$MESS ['F_USER_ID_USER'] = "ID на потребителя";
?>